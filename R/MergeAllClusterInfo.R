...
setwd("/Users/johanreimegard/Vetenskap/Data/clusterProject/ORTHOMCL/")
...
## OrthoMCLInfo
OrthoInfo = read.table(file = 'orthoMCL.parsed.AthalianaOnly.atLeast2.onePerLine.tab.txt2',  sep = "\t")
...
#finalOutput[ finalOutput$OrthoMCLCluster == "ORTHOMCL3620", ]
#finalOutput[ finalOutput$OrthoMCLCluster == "ORTHOMCL2230", ]





simulering = finalOutput[ ,c("Gene","Chr","left","right")]

simulering$STRAND = "FORWARD"
simulering$STRAND[finalOutput$strand=="-"] = "REVERSE"
simulering$Wellmer = 0
simulering$Wellmer[finalOutput$Wellmer2004 >0] = 1
simulering$AlvesFerreria2007 = 0
simulering$AlvesFerreria2007[finalOutput$AlvesFerreria2007 >0] = 1
simulering$Chr = gsub("Chr", "", simulering$Chr)
simulering$array = finalOutput$array


head(simulering)


chromosomes = levels(as.factor(simulering$Chr))
AF = simulering[,c("Gene","Chr","left","right","STRAND","AlvesFerreria2007","array")]
W = simulering[,c("Gene","Chr","left","right","STRAND","Wellmer","array")]

for( i in 1:length(chromosomes)){
  AFtemp = AF[AF$Chr %in% chromosomes[i], ]
  fileName = paste("AlvesFerreria2007",paste("Chr",chromosomes[i],sep = "_"), "expression.tab.txt", sep = "_")
  write.table(AFtemp,file = fileName,sep = "\t", quote = FALSE,row.names = FALSE,col.names=TRUE)

  Wtemp = W[W$Chr %in% chromosomes[i], ]
  fileName = paste("Wellmer2004",paste("Chr",chromosomes[i],sep = "_"), "expression.tab.txt", sep = "_")
  write.table(Wtemp,file = fileName,sep = "\t", quote = FALSE,row.names = FALSE,col.names=TRUE)
}
